...
class PrestadorActiveScreen extends StatefulWidget {
  const PrestadorActiveScreen({super.key});

  @override
  State<PrestadorActiveScreen> createState() => _PrestadorActiveScreenState();
}

class _PrestadorActiveScreenState extends State<PrestadorActiveScreen> {
  ...
  Future<void> _load({bool silent = false}) async {
    ...
    final accepted = all.where((item) => item.isAccepted).toList()
      ..sort((a, b) => b.id.compareTo(a.id));

    if (!mounted) return;

    setState(() {
      _reservations = accepted;
      _isLoading = false;
    });
  }
